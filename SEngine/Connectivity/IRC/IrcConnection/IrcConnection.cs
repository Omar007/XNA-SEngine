	...
	public class IrcConnection
	{
		...
		public bool AutoReconnect
		{
			...
			set
			{
				...
				if (value)
				{
					Console.WriteLine("AutoReconnect enabled");
				}
				else
				{
					Console.WriteLine("AutoReconnect disabled");
				}
#endif
		...
		public bool AutoRetry
		{
			...
			set
			{
				...
				if (value)
				{
					Console.WriteLine("AutoRetry enabled");
				}
				else
				{
					Console.WriteLine("AutoRetry disabled");
				}
#endif
				_AutoRetry = value;
			}
		...
		{
#if DEBUG
			Console.WriteLine("IrcConnection created");
#endif
		...
		~IrcConnection()
		{
			Console.WriteLine("IrcConnection destroyed");
		}
#endif
		...
		public void Connect(string[] addresslist, int port)
		{
			...
			_ConnectTries++;
#if DEBUG
			Console.WriteLine(String.Format("connecting... (attempt: {0})",
												 _ConnectTries));
#endif
...
#if DEBUG
					Console.WriteLine("delaying new connect attempt for " + _AutoRetryDelay + " sec");
#endif
		...
		private bool _WriteLine(string data)
		{
			if (IsConnected)
			{
				...
				{
#if DEBUG
					Console.WriteLine("sending data failed, connection lost");
#endif
					IsConnectionError = true;
					return false;
				}
				...
				{
#if DEBUG
					Console.WriteLine("sending data failed (stream error), connection lost");
#endif
					IsConnectionError = true;
					return false;
				}
...
#if DEBUG
			Console.WriteLine("set server to: " + Address);
#endif
		}
...
#if DEBUG
							Console.WriteLine("PONG received, took: " + _Lag.TotalMilliseconds + " ms");
#endif
							break;
					}
				}
		...
		private class ReadThread
		{
...
#if DEBUG
				Console.WriteLine("Stop(): aborting thread...");
#endif
				_Thread.Abort();
				// make sure we close the stream after the thread is gone, else
				// the thread will think the connection is broken!
#if DEBUG
				Console.WriteLine("Stop(): joining thread...");
#endif
				_Thread.Join();

#if DEBUG
				Console.WriteLine("Stop(): closing reader...");
#endif
				try
				{
			...
			{
#if DEBUG
				Console.WriteLine("ReadThread started");
#endif
				try
				{
					string data = "";
				...
				catch (ThreadAbortException)
				{
					Thread.ResetAbort();
#if DEBUG
					Console.WriteLine("ReadThread aborted");
#endif
				}
				catch (Exception ex)
				{
		...
		private class WriteThread
		{
			...
			{
#if DEBUG
				Console.WriteLine("Stopping WriteThread...");
#endif
			...
			{
#if DEBUG
				Console.WriteLine("WriteThread started");
#endif
				try
				{
					try
				...
				catch (ThreadAbortException)
				{
					Thread.ResetAbort();
#if DEBUG
					Console.WriteLine("WriteThread aborted");
#endif
				}
				catch (Exception ex)
				{
			...
			private bool _CheckHighBuffer()
			{
				if (_HighCount > 0)
				{
					...
					{
#if DEBUG
						Console.WriteLine("Sending data was not sucessful, data is requeued!");
#endif
		...
		private class IdleWorkerThread
		{
			...
			{
#if DEBUG
				Console.WriteLine("IdleWorkerThread started");
#endif
				try
				{
					while (_Connection.IsConnected)
...
#if DEBUG
							Console.WriteLine("ping timeout, connection lost");
#endif
							// only flag this as connection error if we are not
							// cleanly disconnecting
							_Connection.IsConnectionError = true;
				...
				catch (ThreadAbortException)
				{
					Thread.ResetAbort();
#if DEBUG
					Console.WriteLine("IdleWorkerThread aborted");
#endif
				}
				catch (Exception ex)
				{
